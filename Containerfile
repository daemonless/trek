...
# React client -> /build/client/dist.
# The upstream `prebuild` rasterizes PWA icons with sharp (no FreeBSD build);
# do it with rsvg-convert instead, then run vite directly to skip `prebuild`.
RUN cd /build/client && \
    npm ci && \
...
RUN cd /build/server && npm ci --omit=dev
